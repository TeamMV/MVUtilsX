...
public class DynamicByteBuffer implements Cloneable {
    ...
    public DynamicByteBuffer(Byte[] buffer) {
        this.buffer = ArrayUtils.toPrimitive(buffer);
        this.size = buffer.length;
    }
    ...
    public <T extends Number> DynamicByteBuffer push(T[] t) {
        if (t instanceof Byte[]) {
            push(ArrayUtils.toPrimitive((Byte[]) t));
        } else if (t instanceof Integer[]) {
            push(ArrayUtils.toPrimitive((Integer[]) t));
        } else if (t instanceof Long[]) {
            push(ArrayUtils.toPrimitive((Long[]) t));
        } else if (t instanceof Float[]) {
            push(ArrayUtils.toPrimitive((Float[]) t));
        } else if (t instanceof Double[]) {
            push(ArrayUtils.toPrimitive((Double[]) t));
        } else if (t instanceof Short[]) {
            push(ArrayUtils.toPrimitive((Short[]) t));
        }
        return this;
    }
    ...
    public DynamicByteBuffer[] splitInto(int amount) {
        ...
        DynamicByteBuffer[] parts = Sequence.repeating(DynamicByteBuffer::new).take(amount).collect();
